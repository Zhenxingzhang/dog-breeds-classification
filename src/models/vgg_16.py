...
def vgg_16(training_batch, classes, dropout_keep_prob):
    """VGG-like conv-net

    Args:
    training_batch: batch of images (N, 56, 56, 3)
    dropout_keep_prob: training configuration object
...
if __name__ == "__main__":
    ...
    with tf.Session() as sess:
        sess.run(init)
        logits = sess.run(logits, feed_dict={x: image_batch, keep_prob:1.0})
        print(logits)
