...
def mnist_net(x_input_, categories=10, keep_prob_=None):
    ...
    print(out_2.shape)
    out_3 = fc_layer(out_2, num_units=1024, layer_name='FC_1', keep_prob_tensor=keep_prob_)
    print(out_3.shape)
    # out_8 = fc_layer(out_7, num_units=256, layer_name='FC_2', keep_prob_tensor=keep_prob_)
    # print(out_8.shape)
    logits_ = fc_layer(out_3, num_units=categories, layer_name='logits', act=tf.identity, keep_prob_tensor=1.0)

    return logits_


# MODEL
def conv_net(x_input_, categories=200, keep_prob_=None):
    x_input_ = tf.cast(x_input_, tf.float32)
    x_input_ = (x_input_ - 128.0) / 128.0

    out_1 = conv_pool_layer(x_input_, filter_size=3, num_filters=16, layer_name='conv_1', pool=False)
    print(out_1.shape)
